...
export default function App() {
  ...
  const [customModel, setCustomModel] = useState<string>(() => {
    ...
    if (saved === "gemini-3.1-flash-live-preview" || saved === "gemini-3.1-flash-lite-preview") {
      return saved;
    }
  ...
  const playVoicePreview = async (voiceName: string) => {
    ...
    try {
      if (voiceName.startsWith("VOICEVOX_")) {
        ...
        const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-3.1-flash-tts-preview:generateContent?key=${customApiKey}`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            contents: [{ parts: [{ text }] }],
